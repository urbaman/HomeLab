...
```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** sdr list
```

### get fan speeds, exhaust temp  (use dev names from above cmd)

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** sensor reading "Fan1 RPM" "Fan2 RPM" "Fan3 RPM" "Fan4 RPM" "Fan5 RPM" "Fan6 RPM" "Exhaust Temp"
```

### change fan speed control to manual

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** raw 0x30 0x30 0x01 0x00
```

### set fan speed to 30% of max (hint: 0x1e is hexadecimal for 30)

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x1e
```

### set fan speed to 40% of max (hint: 0x28 is hexadecimal for 40)

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x28
```

### change fan speed control to automatic (if you want to go back to letting it manage itself and be unbearably loud again)

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** raw 0x30 0x30 0x01 0x01
```

Check CPU1 temp

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** sdr elist | grep "Temp" | grep "3.1" | cut -c 39-40
```

Check CPU2 temp

```bash
ipmitool -I lanplus -H 192.168.16.2 -U root -P ******** sdr elist | grep "Temp" | grep "3.2" | cut -c 39-40
```
...
# Define variables
activatefile=/etc/fanspeedcontrol/fanspeed.start
oldspeedfile=/etc/fanspeedcontrol/fanspeed.old
changed=0

if grep -Fxq "1" $activatefile
then

  # Set the fan speed control to manual
  ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x01 0x00 >/dev/null

  # Check the CPU temperatures
  cpu1temp=$(ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** sdr elist | grep "Temp" | grep "3.1" | cut -c 39-40)
  cpu2temp=$(ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** sdr elist | grep "Temp" | grep "3.2" | cut -c 39-40)
  ...
  then
    ...
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0xB >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x14 >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x1E >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x28 >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x32 >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x3C >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x46 >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x50 >/dev/null
  ...
  then
    ...
    sed -i "s/$oldspeed/$newspeed/g" $oldspeedfile
    changed=1
    #ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x5A >/dev/null
    #message="CPU1 Temp is $cpu1temp, CPU2 Temp is $cpu2temp; fan speed set to 90%"
    ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x01 0x01 >/dev/null
    message="CPU1 Temp is $cpu1temp, CPU2 Temp is $cpu2temp; fan speed set to dynamic"
    fi
  #elif (( $(bc <<< "$cputemp > 65") > 0 && $(bc <<< "$cputemp <= 67") > 0 ));
  #then
  #  ipmitool -I lanplus -H 10.0.100.21 -U root -P ******** raw 0x30 0x30 0x02 0xff 0x64
